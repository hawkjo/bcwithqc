...
def se_test_lc_output():
    ...
    if USE_TEMP_OUTPUT:
        context = tempfile.TemporaryDirectory(prefix="se_test_lc_pipeline_")
    else:
        root_dir = os.path.join(SCRIPT_DIR, "se_test_lc")
    ...
    with context as root_dir:
        ...
        before_tsv = os.path.join(count_dir, "se_test_lc_before.tsv")
        after_tsv = os.path.join(count_dir, "se_test_lc_after.tsv")
...
def test_simulation_and_count_outputs_exist(se_test_lc_output):
    count_dir = se_test_lc_output["count_dir"]
    ...
    assert os.path.isfile(se_test_lc_output["simulated_fastq"])
    assert os.path.isfile(se_test_lc_output["sans_bc_fastq"])
    assert os.path.isfile(se_test_lc_output["before_tsv"])
    assert os.path.isfile(se_test_lc_output["after_tsv"])


def test_error_combo_tables_are_nonempty(se_test_lc_output):
    for path in [se_test_lc_output["before_tsv"], se_test_lc_output["after_tsv"]]:
