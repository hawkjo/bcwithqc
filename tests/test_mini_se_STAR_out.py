...
se_test_input_dir = os.path.join(SCRIPT_DIR, "../examples/se_test")
...
def outsourced_star_output():
    ...
    if USE_TEMP_OUTPUT:
        context = tempfile.TemporaryDirectory(prefix="se_test_star_outsourcing_")
    else:
        output_dir = os.path.join(SCRIPT_DIR, "se_test_star_outsourcing")
    ...
    with context as output_dir:
        ...
        preprocessed_fastq = os.path.join(output_dir, "sans_bc_se_test.fq")
        if not os.path.isfile(preprocessed_fastq):
            raise FileNotFoundError(...)

        # 2. external STAR alignment
        star_prefix = os.path.join(star_dir, "se_test_")
        star_cmd = [
            'STAR',
            f'--runThreadN 1',
